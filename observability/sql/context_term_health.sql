CREATE VIEW observability.context_term_health AS
WITH signal_rollup AS (
  SELECT business_term_version_id,
  ...
  FROM trust_signals WHERE business_term_version_id IS NOT NULL
  GROUP BY business_term_version_id
), versions AS (
  SELECT btv.*, ROW_NUMBER() OVER (PARTITION BY btv.term_id ORDER BY btv.version_number DESC) = 1 AS is_latest_version
  FROM business_term_versions btv
)
SELECT bt.term_key, bt.canonical_name, bt.domain, v.term_version_id, v.version_number, v.status,
...
JOIN versions v ON v.term_id = bt.term_id
LEFT JOIN governance_owners go ON go.owner_id = bt.owner_id
LEFT JOIN signal_rollup sr ON sr.business_term_version_id = v.term_version_id;
